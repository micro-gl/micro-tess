...
path_t<number> path_star() {
    ...
    static number t2 = number(0.0f);
    t2+=number(0.005f);
    const auto T = number(200) *microgl::math::sin(t2);
//    std::cout << T;
//    a.push_back()
    path_t<number> path{};
    path.lineTo({150, 150})
        .quadraticCurveTo({300, T + 200}, {450, 150})
        .lineTo({200,number(450)})
        .lineTo({300,50})
        .lineTo({400,450})
        .closePath();
    return path;
...
//    using number = Q<15, long long>;
    using number = Q<10, int64_t, int64_t>;
//    using number = Q<2, int64_t>;
//    using number = Q<1, int32_t>;
//    using number = Q<15, int64_t, int64_t>;
//    using number = Q<12>;
